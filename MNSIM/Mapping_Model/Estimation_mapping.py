...
class estimation_mapping(bank):
    def __init__(self, NetStruct_path, SimConfig_path):
        ...
        self.weight_data = torch.load(NetStruct_path)
        self.total_bank_num = len(self.weight_data)
        self.total_layer_num = int(self.weight_data[self.total_bank_num-1][0]['Layernum'])+1
        # print(self.total_bank_num)
        # print(self.total_layer_num)
        self.bank_list = []
    ...
    def config_estimation_mapping(self):
        for i in range(self.total_bank_num):
            layer = int(self.weight_data[i][0]['Layernum'])
            self.bank_num[layer] += 1
            inputsize = int(self.weight_data[i][0]['Inputsize'])
            outputsize = int(self.weight_data[i][0]['Outputsize'])
            kernelsize = int(self.weight_data[i][0]['Kernelsize'])
            stride = int(self.weight_data[i][0]['Stride'])
            inputchannel = int(self.weight_data[i][0]['Inputchannel'])
            output_channel = int(self.weight_data[i][0]['Outputchannel'])
            self.weight_precision[layer] = int(self.weight_data[i][0]['Weightbit'])
            self.activation_precision[layer] = int(self.weight_data[i][0]['Inputbit'])
            self.sliding_times[layer] = outputsize ** 2
            self.operations[layer] += self.sliding_times[layer]*(kernelsize**2)*inputchannel*output_channel*2
            __temp_bank = bank(self.SimConfig_path)
            __temp_bank.bank_read_config(layer_num=layer,activation_precision=self.activation_precision[layer],
                                        sliding_times=self.sliding_times[layer],read_matrix=self.weight_data[i][1])
            self.bank_list[layer].append(__temp_bank)
            # print(i,"ok")
    
    def estimation_mapping_area(self):
        # Notice: before calculating area, config_estimation_mapping must be executed
        # unit: um^2
    ...
    def estimation_mapping_utilization(self):
        # Notice: before calculating utilization, config_estimation_mapping must be executed
        for i in range(self.total_layer_num):
    ...
    def estimation_mapping_latency(self):
        # Notice: before calculating latency, config_estimation_mapping must be executed
        # unit: ns
    ...
    def estimation_mapping_power(self):
        # Notice: before calculating power, config_estimation_mapping must be executed
        # unit: W
        # TODO: add arch level adder tree estimation
        for i in range(self.total_layer_num):
            for j in range(self.bank_num[i]):
    ...
    def estimation_mapping_energy(self):
        # Notice: before calculating energy, config_estimation_mapping must be executed
        # unit: nJ
        # TODO: add arch level adder tree estimation
        for i in range(self.total_layer_num):
            for j in range(self.bank_num[i]):
    ...
    def estimation_mapping_output(self, module_information = 0, layer_information = 0):
...
if __name__ == '__main__':
    # print("ok")
    net_structure_path = os.path.join(os.path.dirname(os.path.dirname(os.getcwd())), "mnsim_weight.pt")
    ...
    _bm = estimation_mapping(net_structure_path, SimConfig_path)
    _bm.config_estimation_mapping()
    _bm.estimation_mapping_area()
    _bm.estimation_mapping_utilization()
    _bm.estimation_mapping_latency()
    _bm.estimation_mapping_power()
    _bm.estimation_mapping_energy()
    _bm.estimation_mapping_output(1, 1)
